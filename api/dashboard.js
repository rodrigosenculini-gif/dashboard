...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (type === 'vendedoras_add_venda') {
      try {
        ...
        // Soma usa o Public ID (UUID) como identificador; nao cabe no bigint.
        // Vai no parametro proprio e a funcao resolve o numero da proposta
        // (proNumBancarizadora) pela consulta que a vendedora acabou de fazer.
        const adesaoTxt = (adesao == null ? '' : String(adesao)).trim();
        const ehNumerica = /^\d+$/.test(adesaoTxt);
        const adesaoNum = ehNumerica ? adesaoTxt : null;
        const idRaw = !ehNumerica && adesaoTxt ? adesaoTxt : null;
        const client = getPool();
        const result = await client.query(
          'select * from dashboard_vendedoras_add_venda($1::text,$2::bigint,$3::text,$4::text,$5::numeric,$6::text,$7::text,$8::date,$9::int,$10::text,$11::text)',
          [vendedor || null, adesaoNum, cpf || null, nome || null, valor || null, banco || null, tabela || null, data_pagamento || null, parcelas || null, seguro || null, idRaw]
        );
        return res.status(200).json(result.rows);
      } catch (e) {
        return res.status(500).json({ error: e.message });
    ...
    if (type === 'novo_saque_saldo') {
      try {
        ...
        let resp;
        try {
          resp = await fetch('https://hotnwh.querosacarfgts.com.br/webhook/novo-saque-saldo', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              cpf, product: product || 'FGTS', vendedor: vendedor || null,
              apenas_consultar: apenas_consultar !== false,
              dados_pagamento: dados_pagamento || null,
              customer_data_manual: customer_data_manual || null,
            }),
            signal: controller.signal,
          });
        } finally {
        ...
        const timeoutMsg = e.name === 'AbortError' ? 'A consulta ao Novo Saque demorou demais. Tente novamente.' : e.message;
        return res.status(500).json({ error: timeoutMsg });
      }
    }
    ...
    const RPC_POST = {
      ...
      vendedoras_buscar_venda: {
        sql: 'select row_to_json(t) as r from dashboard_vendedoras_buscar_venda($1::text,$2::text,$3::text) t',
        args: (b) => [b.vendedor || null, b.adesao || null, b.cpf || null] },
      vendedoras_atualizar_venda: {
        sql: `select row_to_json(t) as r from dashboard_vendedoras_atualizar_venda(
                $1::text,$2::bigint,$3::numeric,$4::text,$5::int,$6::text,$7::date,$8::text,$9::text,$10::text) t`,
        args: (b) => [b.vendedor || null, b.va_id || null, b.valor ?? null, b.tabela || null,
                      b.parcelas ?? null, b.seguro || null, b.data_pagamento || null,
                      b.cpf || null, b.nome || null, b.banco || null] },
    };
    if (RPC_POST[type]) {
      try {
        const client = getPool();
  ...
  if (type === 'count') {
    ...
    // Modelo de fatos: campanha vem do disparo, e interacao/pagamento entram
    // pela atribuicao (ultimo disparo antes do evento), nao pela coluna do lead
    // -- que era sobrescrita a cada novo disparo. Os 4 ultimos parametros sao
    // os filtros de visualizacao, com default no comportamento completo.
    sql = 'select * from dashboard_campanhas_v2($1::text,$2::text,$3::timestamptz,$4::timestamptz,$5::int,$6::int,$7::text,$8::text,$9::text,$10::boolean,$11::boolean,$12::boolean,$13::boolean,$14::text)';
    params = [p_origem, p_meta, p_date_from, p_date_to, p_hora_inicio, p_hora_fim,
              p_tipo_envio, p_mensagem, p_campanha,
              req.body?.so_disparadas === true, req.body?.pago_do_periodo === true,
              req.body?.incluir_tardias !== false, req.body?.incluir_sem_campanha !== false,
